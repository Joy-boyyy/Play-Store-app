...
import './index.css'
import TableItemComp from '../TabItem'
import AppItemComp from '../AppItem'
...
class AppStore extends Component {
  state = {
    menuTitle: appsList[0].category,
    collections: appsList,
    searchResults: appsList,
    selectedTab: tabsList[0].tabId,
  }

  inputFun = element => {
    const {collections} = this.state
    const inputValue = collections.filter(filterPara =>
      filterPara.appName
        .toLowerCase()
  ...
  menuClick = tabId => {
    ...
    const menuClickData = collections.filter(
      filterPara => filterPara.category === tabId,
    )
    this.setState({
      menuTitle: menuClickData[0].category,
      selectedTab: tabId,
    })
  }

  render() {
    const {menuTitle, searchResults, selectedTab} = this.state

    const newFilterArr = searchResults.filter(
      filterPara => filterPara.category === menuTitle,
    )

    return (
      <div className="mainCl">
        <h1 className="h1Cl">App Store</h1>
        ...
        <ul className="ulCl_tblItem">
          {tabsList.map(tblItemMapProp => (
            <TableItemComp
              tblItem={tblItemMapProp}
              key={tblItemMapProp.tabId}
              menuClick={this.menuClick}
              isSelected={tblItemMapProp.tabId === selectedTab}
            />
          ))}
        </ul>

        <ul className="ulCl_appItem">
          {newFilterArr.map(appLstProp => (
            <AppItemComp appLstProp={appLstProp} key={appLstProp.appId} />
          ))}
        </ul>
      </div>
    )
